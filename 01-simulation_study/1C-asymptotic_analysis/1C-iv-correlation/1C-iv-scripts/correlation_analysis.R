...
#' 2. differing levels of correlation [u_list]
#' 
#' Procedure:
...
#' 
#' Notes:
#' Correlation is determined via u parameter:
#'  First we simulate a fully connected graph,
#'  we set partial correlations to be 0.1,
#'  when making the matrix positive definite via min_eigenvalue, 
#'  we add u to the diagonal entries in the precision matrix,
#'  the higher the u, the smaller the correlation. 
...
u_list <- 10^seq(-5, 0, by = 1) # expected proportion of explained variance
k <- 1:100 # iterations

## Parameter combinations 

params_grid <- expand.grid(k, u_list, pk, n)
colnames(params_grid) <- c("k","u_list", "pk", "n")
...
simulate_data <- function(params){
  ...
  n_tmp <- params$n
  pk_tmp <- params$pk
  u_list_tmp <- params$u_list
  
  # Set seed based on iteration k
  set.seed(k_tmp)
  
  # Simulate data
  xdata <- SimulateGraphical(n = n_tmp, 
                             pk = pk_tmp,
                             nu_within = 1,
                             v_within = 0.1, 
  ...
  sim_int <- SimulateInteraction(xdata = xdata$data,
                                 ...
                                 beta_abs = 1, # vector defining the range of nonzero regression coefficients
...
iterate <- function(params_grid, n_cores, node){
  ...
  saveRDS(sim_int, file = paste0("../1C-iv-data/sim_",node,".rds"))
  
  # Perform models
  
  # LASSO
  stab_z <- perform_stab(sim_int = sim_int, n_cat = NULL, n_cores = n_cores, pi_list = seq(0.01, 0.99, by = 0.01))
  saveRDS(stab_z, file= paste0("../1C-iv-results/1C-iv-models/lasso_stab_zscore_",node,".rds"))
  
  stab_nll <- perform_stab(sim_int = sim_int, n_cat = 3, n_cores = n_cores, pi_list = seq(0.5, 0.99, by = 0.01))
  saveRDS(stab_nll, file= paste0("../1C-iv-results/1C-iv-models/lasso_stab_nll_",node,".rds"))
  
  cv <- perform_cv(sim_int = sim_int)
  saveRDS(cv, file= paste0("../1C-iv-results/1C-iv-models/lasso_cv_",node,".rds"))
  
  # GLINTERNET
  stab_int_z <- perform_stab_int(sim_int = sim_int, n_cat = NULL, n_cores = n_cores, pi_list = seq(0.01, 0.99, by = 0.01)) 
  saveRDS(stab_int_z, file= paste0("../1C-iv-results/1C-iv-models/glinternet_stab_zscore_",node,".rds"))
  
  stab_int_nll <- perform_stab_int(sim_int = sim_int, n_cat = 3, n_cores = n_cores, pi_list = seq(0.5, 0.99, by = 0.01))
  saveRDS(stab_int_nll, file= paste0("../1C-iv-results/1C-iv-models/glinternet_stab_nll_",node,".rds"))
  
  cv_int <- perform_cv_int(sim_int = sim_int)
  saveRDS(cv_int, file= paste0("../1C-iv-results/1C-iv-models/glinternet_cv_",node,".rds"))
  ...
  # Add parameters
  performance$k <- k_tmp
  performance$u_list <- u_list_tmp
  performance$pk <- pk_tmp
  performance$n <- n_tmp
  rownames(performance) <- NULL
  
  saveRDS(performance, file= paste0("../1C-iv-results/1C-iv-performances/performance_",node,".rds"))
...
correlation_analysis <- iterate(params_grid = params_grid,
                                n_cores = n_cores,
                                node = node)
