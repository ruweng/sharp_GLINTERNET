...
#' 2. differing expected probabilties of interactions [nu_int]
#' 
#' Procedure:
...
nu_int <- seq(1, 0, by = -0.1)# expected proportion of explained variance
k <- 1:100 # iterations

## Parameter combinations 

params_grid <- expand.grid(k, nu_int, pk, n)
colnames(params_grid) <- c("k","nu_int", "pk", "n")
...
simulate_data <- function(params){
  ...
  sim_int <- SimulateInteraction(n = n_tmp, # number of observations
                                 ...
                                 nu_int = nu_int_tmp, # expected probability of interaction effect
...
iterate <- function(params_grid, n_cores, node){
  ...
  saveRDS(sim_int, file = paste0("../1C-i-data/sim_",node,".rds"))
  
  # Perform models
  
  # LASSO
  stab_z <- perform_stab(sim_int = sim_int, n_cat = NULL, n_cores = n_cores, pi_list = seq(0.01, 0.99, by = 0.01))
  saveRDS(stab_z, file= paste0("../1C-i-results/1C-i-models/lasso_stab_zscore_",node,".rds"))
  
  stab_nll <- perform_stab(sim_int = sim_int, n_cat = 3, n_cores = n_cores, pi_list = seq(0.5, 0.99, by = 0.01))
  saveRDS(stab_nll, file= paste0("../1C-i-results/1C-i-models/lasso_stab_nll_",node,".rds"))
  
  cv <- perform_cv(sim_int = sim_int)
  saveRDS(cv, file= paste0("../1C-i-results/1C-i-models/lasso_cv_",node,".rds"))
  
  # GLINTERNET
  stab_int_z <- perform_stab_int(sim_int = sim_int, n_cat = NULL, n_cores = n_cores, pi_list = seq(0.01, 0.99, by = 0.01)) 
  saveRDS(stab_int_z, file= paste0("../1C-i-results/1C-i-models/glinternet_stab_zscore_",node,".rds"))
  
  stab_int_nll <- perform_stab_int(sim_int = sim_int, n_cat = 3, n_cores = n_cores, pi_list = seq(0.5, 0.99, by = 0.01))
  saveRDS(stab_int_nll, file= paste0("../1C-i-results/1C-i-models/glinternet_stab_nll_",node,".rds"))
  
  cv_int <- perform_cv_int(sim_int = sim_int)
  saveRDS(cv_int, file= paste0("../1C-i-results/1C-i-models/glinternet_cv_",node,".rds"))
  ...
  # Add parameters
  performance$k <- k_tmp
  performance$nu_int <- nu_int_tmp
  performance$pk <- pk_tmp
  performance$n <- n_tmp
  rownames(performance) <- NULL
  
  saveRDS(performance, file= paste0("../1C-i-results/1C-i-performances/performance_",node,".rds"))
...
sparsity_analysis <- iterate(params_grid = params_grid,
                             n_cores = n_cores,
                             node = node)
