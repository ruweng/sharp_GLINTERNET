...
#' 2. differing levels of noise [expected proportion variance explained]
#' 
#' Procedure:
...
## Parameter combinations 
  ...
  params_grid <- expand.grid(k, ev_xy, pk, n)
  colnames(params_grid) <- c("k","ev_xy", "pk", "n")
...
iterate <- function(params_grid, n_cores, node){
      ...
      saveRDS(sim_int, file = paste0("../1C-ii-data/sim_",node,".rds"))
      
      # Perform models
      
      # LASSO
      stab_z <- perform_stab(sim_int = sim_int, n_cat = NULL, n_cores = n_cores, pi_list = seq(0.01, 0.99, by = 0.01))
      saveRDS(stab_z, file= paste0("../1C-ii-results/1C-ii-models/lasso_stab_zscore_",node,".rds"))
      
      stab_nll <- perform_stab(sim_int = sim_int, n_cat = 3, n_cores = n_cores, pi_list = seq(0.5, 0.99, by = 0.01))
      saveRDS(stab_nll, file= paste0("../1C-ii-results/1C-ii-models/lasso_stab_nll_",node,".rds"))
      
      cv <- perform_cv(sim_int = sim_int)
      saveRDS(cv, file= paste0("../1C-ii-results/1C-ii-models/lasso_cv_",node,".rds"))
      
      # GLINTERNET
      stab_int_z <- perform_stab_int(sim_int = sim_int, n_cat = NULL, n_cores = n_cores, pi_list = seq(0.01, 0.99, by = 0.01)) 
      saveRDS(stab_int_z, file= paste0("../1C-ii-results/1C-ii-models/glinternet_stab_zscore_",node,".rds"))
      
      stab_int_nll <- perform_stab_int(sim_int = sim_int, n_cat = 3, n_cores = n_cores, pi_list = seq(0.5, 0.99, by = 0.01))
      saveRDS(stab_int_nll, file= paste0("../1C-ii-results/1C-ii-models/glinternet_stab_nll_",node,".rds"))
      
      cv_int <- perform_cv_int(sim_int = sim_int)
      saveRDS(cv_int, file= paste0("../1C-ii-results/1C-ii-models/glinternet_cv_",node,".rds"))
      ...
      # Add parameters
      performance$k <- k_tmp
      performance$ev_xy <- ev_xy_tmp
      performance$pk <- pk_tmp
      performance$n <- n_tmp
      rownames(performance) <- NULL
      
      saveRDS(performance, file= paste0("../1C-ii-results/1C-ii-performances/performance_",node,".rds"))
...
noise_analysis <- iterate(params_grid = params_grid,
                          n_cores = n_cores,
                          node = node)
